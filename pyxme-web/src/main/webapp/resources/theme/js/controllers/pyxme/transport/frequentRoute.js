...
app.controller("FrequentRoute",['$state','$modal','utilStringService','dispatcherCarService','NgMap','$geolocation',"$scope", 'Alertify', 'blockUI','$q',
                     function($state,$modal,utilStringService,   dispatcherCarService,   NgMap,   $geolocation,   $scope,  Alertify,   blockUI,  $q) {
    controller = this;
	...
	controller.guardarObjeto = function (objDto){
		...
		respuesta.then(function (data){
			...
			encerar ();
			utilStringService.crearMensaje($scope, 1, "Exito servicio guardado");
		},function(error){
			blockUI.stop();
			encerar ();
			$scope.currentModal.dismiss();
			utilStringService.crearMensaje($scope, 3, "Error");
		});
	};
